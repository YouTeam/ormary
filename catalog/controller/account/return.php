...
class ControllerAccountReturn extends Controller {
    ...
    public function insert() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            $mail = new Mail();
            $mail->sendNotificationEmail('andrew@ormary.com', '', 'RETURN REQUESTED ' . $this->request->post['order_id'] . '');
        ...
        $my_orders = $this->model_account_order->getOrders(0, 1000);
        $my_order_with_products;
        foreach ($my_orders as $key => $order) {
            $order_products = $this->model_account_order->getOrderProductsWithDetails($order['order_id']);
            $order['order_products'] = $order_products;

            $my_order_with_products[$key] = $order;
        }

        $this->data['my_orders'] = $my_order_with_products;
