...
$this->data['share_prod_friend']=$this->url->link('product/product/emailshare', 'product_id='.$this->request->get['product_id']);
			...
			{
				$this->data['user_firstname'] =  $this->customer->getFirstName();
				$this->data['user_email'] = $this->customer->getEmail();
			}
			else
			{
			...
			foreach ($results as $result) {
				$this->data['images'][] = array(
					...
					'thumb' => $this->model_tool_image->resize($result['image'], $this->config->get('config_image_thumb_width'), $this->config->get('config_image_thumb_height')),
                                     'mainimage' => $this->model_tool_image->resize($result['image'], $this->config->get('config_image_product_width'), $this->config->get('config_image_product_height'))
				);
			}
			...
			{ 
				...
				$this->data['category_info'] = array(
					...
					'href'      => $this->url->link('product/category', 'path=' .  $this->model_catalog_category->getCategoryPath($categories[0]['category_id'])),
					'separator' => $this->language->get('text_separator')
				);
			}
			...
			if($this->customer->isLogged())
			{
				...
				if($this->model_account_follow->checkFollow($product_info['manufacturer_id'], $this->customer->getId()))
				{
					$this->data['follow_state'] = "unfollow";
				}
				else
				{
					$this->data['follow_state'] = "follow";
				}
			}
			else
			{
				$this->data['follow_state'] = "";
			}
			...
			foreach ($this->model_catalog_product->getProductOptions($this->request->get['product_id']) as $option) {
				if ($option['type'] == 'select' || $option['type'] == 'radio' || $option['type'] == 'checkbox' || $option['type'] == 'image') {
					...
					$this->data['options'][$option['option_id']] = array(
	...
	public function emailshare()
	{
		...
		$email_message = str_ireplace("MESSAGE_TEXT", $msg." ".$url, $email_message);
		
		$mail = new Mail();
		$mail->sendNotificationEmail($email, '', $email_message, $subj);
