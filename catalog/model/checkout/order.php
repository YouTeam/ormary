...
/*
			...
			// Admin Alert Mail - this notification is not working. TODO: fix smtp and configure notification
			if ($this->config->get('config_alert_mail') && false) {
	...
	public function update($order_id, $order_status_id, $comment = '', $notify = false) {
		...
		if ($order_info && $order_info['order_status_id']) {
			...
			$email_message = file_get_contents('./emails/status_template.html', FILE_USE_INCLUDE_PATH);
			$status_query = $this->db->query("select * from ".DB_PREFIX."order_status where order_status_id = ".(int)$order_status_id);
			if ($status_query->num_rows) {
				$status_name = $status_query->row['name'];
				}
			$email_message = str_ireplace("CUSTOMER_NAME", $order_info['firstname'], $email_message);
			$email_message = str_ireplace("ORDER_STATUS", strtolower($status_name), $email_message);
			$mail = new Mail();
			$mail->sendNotificationEmail($order_info['email'], '', $email_message, 'ORMARY order update');
			// Send out any gift voucher mails
			if ($this->config->get('config_complete_status_id') == $order_status_id) {
				$this->load->model('checkout/voucher');
