...
class Mail {
	...
	public function send() {
		...
		if (is_array($this->to)) {
			...
			$api_key="key-48748180cc1d92b2be1a4d4b646cdea5";
			 $domain ="ormary.com";/* Domain Name you given to Mailgun */
			 $ch = curl_init();
			 ...
			 curl_setopt($ch, CURLOPT_POSTFIELDS, array(
			  'from' => 'Ormary customer service <care@ormary.com>',
			  'to' => $this->to,
...
/*
	...
	public function sendNotificationEmail( $emailTo, $template='', $message , $subject)
	{
			//$message  = file_get_contents('./emails/welcome_template.html', FILE_USE_INCLUDE_PATH);
			$config = new Config();
			$config->load('smtp');
			
/*			$this->data['server'] = $config->get('ORMARY_SERVER');
			if($template != '')
			{
			...
			$message = str_ireplace('ORMARY_SERVER', $config->get('ORMARY_SERVER'), $message);	
			...
			$this->setTo($emailTo);
			$this->setFrom('care@ormary.com');
			$this->setSender('Ormary customer service.');
			$this->setSubject('Welcome to ORMARY');					
