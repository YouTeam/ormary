...
class ControllerCatalogManufacturer extends Controller { 
	...
	protected function getForm() {
		...
		if (isset($this->request->post['manufacturer_style'])) {
			$this->data['manufacturer_style'] = $this->request->post['manufacturer_style'];
		} elseif (isset($this->request->get['manufacturer_id'])) {
			$this->data['manufacturer_style'] = $manufacturer_info['style_id'];
		} else {
			$this->data['manufacturer_style'] = 0;
		}	
		
		$this->data['manufacturer_styles_list'] = $this->model_catalog_manufacturer->getManufacturersStyles();
		...
		if (isset($this->request->post['bgimage'])) {
			$this->data['bgimage'] = $this->request->post['bgimage'];
		} elseif (!empty($manufacturer_info)) {
			$this->data['bgimage'] = $manufacturer_info['bgimage'];
		} else {
			$this->data['bgimage'] = '';
		}
		
		
		if (isset($this->request->post['bgimage']) && file_exists(DIR_IMAGE . $this->request->post['bgimage'])) {
			$this->data['bgthumb'] = $this->model_tool_image->resize($this->request->post['bgimage'], 500, 100);
		} elseif (!empty($manufacturer_info) && $manufacturer_info['bgimage'] && file_exists(DIR_IMAGE . $manufacturer_info['bgimage'])) {
			$this->data['bgthumb'] = $this->model_tool_image->resize($manufacturer_info['bgimage'], 500, 100);
		} else {
			$this->data['bgthumb'] = $this->model_tool_image->resize('no_image.jpg', 100, 100);
		}

		$this->data['no_bgimage'] = $this->model_tool_image->resize('no_image.jpg', 100, 100);
