...
$(document).ready(function(){
	...
	<?php 
	$allowedArray = "var allowed = new Array(";
  foreach ($this->user->getAllowedRoutes() as $acc){ $allowedArray .= "'".$acc."',"; } 
  $allowedArray .= "'');";
  echo $allowedArray;
  ?>
	$('#menu .left').find('li').each(function(index){ 
		if($(this).children("ul").length){;}
		else{
			isAllowed = false;
			for( i = 0; i < allowed.length; i++){
				if($(this).find('a').attr('href').indexOf(allowed[i]) > 0){isAllowed = true;}
			}
			if(!isAllowed)$(this).remove();
			}
	});

	$('#menu .left').find('ul').each(function(index){if($(this).children("li").length == 0){$(this).remove();}});
	$('#menu .left').find('li a.parent').each(function(index){if($(this).parent().children().length == 1){$(this).parent().remove();}});
	$('#menu .left').find('li').each(function(index){if($(this).children().length == 0){$(this).remove();}});
	$('#menu .left').find('ul').each(function(index){if($(this).children().length == 0){$(this).remove();}});
	$('#menu .left').find('li a.top').each(function(index){if($(this).parent().children().length == 1){$(this).parent().remove();}});
       });
    </script>
</head>
<body>
<div id="container" class="wrapper">
    <div id="header" class="container-fluid top_bar">
  <div class="container">
    <!--<div class="div2"><img src="view/image/logo.png" title="<?php echo $heading_title; ?>" onclick="location = '<?php echo $home; ?>'" /></div>-->
    <?php if ($logged) { ?>
    <div class="col-lg-6 col-md-5 left_side clearfix" style="float:left;"><img src="<?php echo $this->url->img_link('../image/data/ormary/logo.png');?>" alt="" style="position: relative; top: 3px; left:20px;" /></div>
	<div id="welcome" class="navbar-right right_side clearfix" style="float:right;padding-top:15px;padding-right:15px;"><span style="color:white;"><?php echo $logged; ?></span></div>
    <?php } ?>
  </div>
  <?php if ($logged) { ?>
    ...
    <ul class="left" style="display: none;">
      ...
      <li id="system"><a class="top"><?php echo $text_system; ?></a>
        <ul>
          ...
          <li><a href="<?php echo $landing_page; ?>">Landing page</a></li>
