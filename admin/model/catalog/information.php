...
class ModelCatalogInformation extends Model {
	...
	public function editInformation($information_id, $data) {
		...
		foreach ($data['information_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "information_description SET information_id = '" . (int)$information_id . "', language_id = '" . (int)$language_id . "', title = '" . $this->db->escape($value['title']) . "', description = '" . $this->db->escape($value['description']) . "', img_url = '" . $this->db->escape($data['image']) . "'");
		}
	...
	public function getTeamMembers(){
		$query = $this->db->query("SELECT Id as id, IFNULL(Name, '') as name, IFNULL(Position, '') as position, IFNULL(FB_URL, '') as fb_url, IFNULL(TWI_URL, '') as twi_url, IFNULL(LI_URL, '') as li_url, IFNULL(Bio, '') as bio, IFNULL(Photo_url, '') as photo_url    FROM " . DB_PREFIX . "team t ORDER BY t.name ASC");
		
		return $query->rows;
	}
	
	public function getOffices(){
		$query = $this->db->query("SELECT Id as id, IFNULL(Name, '') as name, IFNULL(addr, '') as addr FROM " . DB_PREFIX . "offices t ORDER BY t.name ASC");
		
		return $query->rows;
	}	
	
	public function addTeamMember ($data){
				$this->db->query("INSERT INTO " . DB_PREFIX . "team (name, position, bio, fb_url, twi_url, li_url, photo_url) values('".$data['name']."','".$data['position']."','".$data['bio']."','".$data['fb_url']."','".$data['twi_url']."','".$data['li_url']."','".$data['image']."')");
	}
	
	public function updateTeamMember ($id, $data){
				$this->db->query("UPDATE " . DB_PREFIX . "team set Name ='".$data['name']."', Position = '".$data['position']."', Bio = '".$data['bio']."', FB_URL = '".$data['fb_url']."', TWI_URL = '".$data['twi_url']."', LI_URL = '".$data['li_url']."', Photo_url = '".$data['image']."' where Id = ".$id);
	}
	
	public function deleteTeamMember ($id){
				$this->db->query("DELETE FROM " . DB_PREFIX . "team  where Id = ".$id);
	}
	
	public function addOffice ($data){
				$this->db->query("INSERT INTO " . DB_PREFIX . "offices (name, addr) values('".$data['name']."','".$data['addr']."')");
	}
	
	public function updateOffice ($id, $data){
				$this->db->query("UPDATE " . DB_PREFIX . "offices set name ='".$data['name']."', addr = '".$data['addr']."' where Id = ".$id);
	}
	
	public function deleteOffice ($id){
				$this->db->query("DELETE FROM " . DB_PREFIX . "offices  where Id = ".$id);
	}
}
?>